...
def auto_compile_uis(ui_dir):
    ...
    for ui_path in ui_paths:
        ...
        if ui_name != class_name:
            ...
            temp_ui_file.write(etree.tostring(xml_tree.getroot(), encoding="utf-8"))
            temp_ui_file.close()
        
        compiled_ui_path = os.path.join(ui_dir, "Ui_%s.py" % ui_name)
        compie_command = "pyside-uic %s -o %s" % (temp_ui_file_path, compiled_ui_path)
...
def import_uis():
    ...
    for suffix in suffixs:
        compiled_ui_paths += glob.glob(os.path.join(outer_module_dir, "Ui_*.%s" % suffix))

    for compiled_ui_path in compiled_ui_paths:
        module_name = os.path.splitext(os.path.basename(compiled_ui_path))[0][3:]
